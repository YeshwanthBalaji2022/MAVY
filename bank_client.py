...
def deposit():
    ...
    data = f"{ac},{pin},{amount}"
    try:
        client_socket.send(data.encode())
        response = client_socket.recv(1024).decode()
        return render_template("depo.html",updatedbalance = response)  
    except:
        return render_template("error.html")
...
def deleteAccount():
    ...
    try:
        client_socket.send(data.encode())
        return render_template("deleteAccount.html",delout = "Account Deleted Successfully")  
    except:
        return render_template("error.html")       
